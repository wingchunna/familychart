...
(function (global, factory) {
    ...
    <g class="card-body">
      ...
      <g transform="translate(${card_dim.text_x}, ${card_dim.text_y})">
        <text  clip-path="url(#card_text_clip) ">
          <tspan  text-anchor="middle" x="${100}"  dy="${20}" id='name'>${card_display[0](d.data)}</tspan>
          <tspan  text-anchor="middle" x="${100}" dy="${20}" font-size="12">${card_display[1](d.data)}</tspan>
        </text>
  ...
  function CardBodyOutline({ d, card_dim, is_new }) {
    return {
      template: (`
    <rect width="${card_dim.w}" height="${card_dim.h}" rx="10" ry="10" class="card-outline ${(d.data.main && !is_new) ? 'card-main-outline' : ''} ${is_new ? 'card-new-outline' : ''}" />
  `)
    }
  }
      ...
      <g transform="translate(${x || card_dim.w / 2},${y || card_dim.h})scale(.13)">
        ...
        <g transform="translate(-10, -8)">
          <line
            ...
            stroke="#fff" stroke-width="15" stroke-linecap="round"
          />
          <line
  ...
  function CardImage({ d, image, card_dim, maleIcon, femaleIcon }) {
    ...
    <g style="transform: translate(${card_dim.img_x}px,${card_dim.img_y}px);" class="card_image" clip-path="url(#card_image_clip)">
      ${image
          ? `<image href="${image}" height="${card_dim.img_h}" width="${card_dim.img_w}" preserveAspectRatio="xMidYMin slice" clip-path="inset(0% round 15px)" />`
    ...
    `)
      ...
      function CardBody({ d, w, h }) {
        ...
        return {
          template: (`
        <g>
          <rect width="${w}" height="${h}" fill="#ffffff" rx="${10}" ${d.data.main ? 'stroke="#ffffff"' : ''} class="${color_class}" />
  ...
  function Card(props) {
    ...
    function setupSvgDefs() {
      ...
      <defs id="f3CardDef">
          ...
          <pattern id="cardBackground" x="0" y="0" width="210" height="120"
          patternUnits="userSpaceOnUse">
            <image x="0" y="0" width="210" height="120" id="imgBG";
                    xlink:href="https://png.pngtree.com/thumb_back/fw800/background/20190223/ourmid/pngtree-classical-pattern-advertising-background-designbeautiful-backgroundchinese-stylecreative-image_72819.jpg" />
          </pattern>
       <mask id="fade" maskContentUnits="objectBoundingBox"><rect width="1" height="1" fill ="url(#cardBackground)"/></mask>
    ...
    function setupProps(props) {
      const default_props = {
        mini_tree: true,
        link_break: true,
        card_dim: { w: 220, h: 120, text_x: 75, text_y: 15, img_w: 60, img_h: 60, img_x: 5, img_y: 5 }
      };
      if (!props) props = {};
    ...
    function createSvg() {
      const svg_dim = cont.getBoundingClientRect(),
        ...
        <pattern id="img1" patternUnits="userSpaceOnUse" x="0" y="0" width="1000" height="700">
                    <image xlink:href="" width="600" height="450" />             
            </pattern>
    </defs>
    <path d="M5,5
